...
for eps in range(0,stcounter_max+1):
   ...
   for chi_counter in range(0,chicounter_max+1):  
      ...
      print(f'Solving eps = {st} and chi = {chi}')
      ...
      for i in range(0, counter_max+1):
        ...
        uHS = (8.0*volfraction[i]*alfa - 9.0*(volfraction[i]*alfa)**2.0 + 3.0*(volfraction[i]*alfa)**3.) 
        uHS = uHS/(1. - alfa*volfraction[i])**3.
        ...
        F_HS.append(nbeads*rho[i]*alfa*volfraction[i] * (4.0-3.0 * alfa*volfraction[i])/(1 - alfa*volfraction[i])**2.0)
    ...
    #  plt.scatter(volfractiontoplot,muwatertoplot,s=5,label=f'eps = {st:.2f}')
      ...
      if len(mu_limits) == 2:
        rho_towrite=np.linspace(rho[0],rho_eq[0],10000)
        mu_towrite=mudens_interp(rho_towrite)
